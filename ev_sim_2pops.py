...
def main():
    ...
    t = 0
    while (test_convergence(p, old_ps) <= 0.75 or test_convergence(p2, old_ps2) <= 0.75):
        print(f"Time={t}: strategies: {np.round(p, 3)}")
        t += 1
        old_ps = p
        old_ps2 = p2
    ...
    plt.plot(means11, color="xkcd:orange", label="mean p1(H), sig 1")
    plt.plot(means12, "y", label="mean p1(H), sig 2")
    plt.plot(means21, color="xkcd:blue", label="mean p2(H), sig 1")
    plt.plot(means22, "g", label="mean p2(H), sig 2")
